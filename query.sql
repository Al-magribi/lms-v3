...
CREATE TABLE l_reports(
    ...
    createdat timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(id),
    CONSTRAINT l_reports_periodeid_fkey FOREIGN key(periodeid) REFERENCES a_periode(id),
    CONSTRAINT l_reports_classid_fkey FOREIGN key(classid) REFERENCES a_class(id),
    CONSTRAINT l_reports_subjectid_fkey FOREIGN key(subjectid) REFERENCES a_subject(id),
    CONSTRAINT l_reports_studentid_fkey FOREIGN key(studentid) REFERENCES u_students(id),
    CONSTRAINT l_reports_teacherid_fkey FOREIGN key(teacherid) REFERENCES u_teachers(id),
    CONSTRAINT l_reports_month_check CHECK ((month = ANY (ARRAY['Januari'::text, 'Februari'::text, 'Maret'::text, 'April'::text, 'Mei'::text, 'Juni'::text, 'Juli'::text, 'Agustus'::text, 'September'::text, 'Oktober'::text, 'November'::text, 'Desember'::text]))),
    CONSTRAINT l_reports_performance_check CHECK ((performance = ANY (ARRAY['A'::text, 'B'::text, 'C'::text]))),
    CONSTRAINT l_reports_discipline_check CHECK ((discipline = ANY (ARRAY['A'::text, 'B'::text, 'C'::text]))),
    CONSTRAINT l_reports_activeness_check CHECK ((activeness = ANY (ARRAY['A'::text, 'B'::text, 'C'::text]))),
    CONSTRAINT l_reports_confidence_check CHECK ((confidence = ANY (ARRAY['A'::text, 'B'::text, 'C'::text])))
);
...
CREATE TABLE l_scores(
    ...
    createdat timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(id),
    CONSTRAINT l_scores_contentid_fkey FOREIGN key(contentid) REFERENCES l_content(id)
);
