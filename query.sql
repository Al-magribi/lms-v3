-- Active: 1747900993750@@212.85.24.143@5432@lmsv3
...
CREATE TABLE l_attendance(
    ...
    classid integer REFERENCES a_class(id) ON DELETE CASCADE   ,
...
-- ========================================
-- NEW TABLES FOR ATTITUDE, FORMATIVE, AND SUMMATIVE SCORES
-- ========================================

-- Table for Attitude Scores (Penilaian Sikap)
CREATE TABLE l_attitude(
    ...
    kinerja INTEGER CHECK (kinerja >= 0 AND kinerja <= 100),
    kedisiplinan INTEGER CHECK (kedisiplinan >= 0 AND kedisiplinan <= 100),
    keaktifan INTEGER CHECK (keaktifan >= 0 AND keaktifan <= 100),
    percaya_diri INTEGER CHECK (percaya_diri >= 0 AND percaya_diri <= 100),
    catatan_guru TEXT,
...
-- Table for Formative Scores (Penilaian Formatif)
CREATE TABLE l_formative(
    ...
    semester INTEGER CHECK (semester >= 1 AND semester <= 2),
    month VARCHAR(20) CHECK (month IN ('Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember')),
    F_1 INTEGER CHECK (F_1 >= 0 AND F_1 <= 100),
    F_2 INTEGER CHECK (F_2 >= 0 AND F_2 <= 100),
    F_3 INTEGER CHECK (F_3 >= 0 AND F_3 <= 100),
    F_4 INTEGER CHECK (F_4 >= 0 AND F_4 <= 100),
    F_5 INTEGER CHECK (F_5 >= 0 AND F_5 <= 100),
    F_6 INTEGER CHECK (F_6 >= 0 AND F_6 <= 100),
    F_7 INTEGER CHECK (F_7 >= 0 AND F_7 <= 100),
    F_8 INTEGER CHECK (F_8 >= 0 AND F_8 <= 100),
    rata_rata NUMERIC(5,2),
    createdat TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updatedat TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Table for Summative Scores (Penilaian S)
CREATE TABLE l_summative(
    ...
    semester INTEGER CHECK (semester >= 1 AND semester <= 2),
    month VARCHAR(20) CHECK (month IN ('Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember')),
    S_1 INTEGER CHECK (S_1 >= 0 AND S_1 <= 100),
    S_2 INTEGER CHECK (S_2 >= 0 AND S_2 <= 100),
    S_3 INTEGER CHECK (S_3 >= 0 AND S_3 <= 100),
    S_4 INTEGER CHECK (S_4 >= 0 AND S_4 <= 100),
    S_5 INTEGER CHECK (S_5 >= 0 AND S_5 <= 100),
    S_6 INTEGER CHECK (S_6 >= 0 AND S_6 <= 100),
    S_7 INTEGER CHECK (S_7 >= 0 AND S_7 <= 100),
    S_8 INTEGER CHECK (S_8 >= 0 AND S_8 <= 100),
    rata_rata NUMERIC(5,2),
    createdat TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updatedat TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_attitude_student_subject ON l_attitude(student_id, subject_id);
CREATE INDEX idx_attitude_class_chapter ON l_attitude(class_id, chapter_id);
CREATE INDEX idx_attitude_month ON l_attitude(month);

CREATE INDEX idx_formative_student_subject ON l_formative(student_id, subject_id);
CREATE INDEX idx_formative_class_chapter ON l_formative(class_id, chapter_id);
CREATE INDEX idx_formative_month ON l_formative(month);

CREATE INDEX idx_summative_student_subject ON l_summative(student_id, subject_id);
CREATE INDEX idx_summative_class_chapter ON l_summative(class_id, chapter_id);
CREATE INDEX idx_summative_month ON l_summative(month);


-- Create unique constraints to prevent duplicate entries
CREATE UNIQUE INDEX idx_attitude_unique ON l_attitude(student_id, subject_id, class_id, chapter_id, month);
CREATE UNIQUE INDEX idx_formative_unique ON l_formative(student_id, subject_id, class_id, chapter_id, month);
CREATE UNIQUE INDEX idx_summative_unique ON l_summative(student_id, subject_id, class_id, chapter_id, month);

-- Create triggers to automatically calculate averages
CREATE OR REPLACE FUNCTION calculate_attitude_average()
RETURNS TRIGGER AS $$
BEGIN
    NEW.rata_rata = (
        COALESCE(NEW.kinerja, 0) + 
        COALESCE(NEW.kedisiplinan, 0) + 
        COALESCE(NEW.keaktifan, 0) + 
        COALESCE(NEW.percaya_diri, 0)
    ) / 4.0;
...
CREATE TRIGGER trigger_calculate_attitude_average
    BEFORE INSERT OR UPDATE ON l_attitude
    FOR EACH ROW
    EXECUTE FUNCTION calculate_attitude_average();

CREATE OR REPLACE FUNCTION calculate_formative_average()
RETURNS TRIGGER AS $$
DECLARE
    total_score INTEGER := 0;
    score_count INTEGER := 0;
BEGIN
    -- Calculate average only for non-null scores
    IF NEW.F_1 IS NOT NULL THEN total_score := total_score + NEW.F_1; score_count := score_count + 1; END IF;
    IF NEW.F_2 IS NOT NULL THEN total_score := total_score + NEW.F_2; score_count := score_count + 1; END IF;
    IF NEW.F_3 IS NOT NULL THEN total_score := total_score + NEW.F_3; score_count := score_count + 1; END IF;
    IF NEW.F_4 IS NOT NULL THEN total_score := total_score + NEW.F_4; score_count := score_count + 1; END IF;
    IF NEW.F_5 IS NOT NULL THEN total_score := total_score + NEW.F_5; score_count := score_count + 1; END IF;
    IF NEW.F_6 IS NOT NULL THEN total_score := total_score + NEW.F_6; score_count := score_count + 1; END IF;
    IF NEW.F_7 IS NOT NULL THEN total_score := total_score + NEW.F_7; score_count := score_count + 1; END IF;
    IF NEW.F_8 IS NOT NULL THEN total_score := total_score + NEW.F_8; score_count := score_count + 1; END IF;
...
CREATE TRIGGER trigger_calculate_formative_average
    BEFORE INSERT OR UPDATE ON l_formative
    FOR EACH ROW
    EXECUTE FUNCTION calculate_formative_average();

CREATE OR REPLACE FUNCTION calculate_summative_average()
RETURNS TRIGGER AS $$
DECLARE
    total_score INTEGER := 0;
    score_count INTEGER := 0;
BEGIN
    -- Calculate average only for non-null scores
    IF NEW.S_1 IS NOT NULL THEN total_score := total_score + NEW.S_1; score_count := score_count + 1; END IF;
    IF NEW.S_2 IS NOT NULL THEN total_score := total_score + NEW.S_2; score_count := score_count + 1; END IF;
    IF NEW.S_3 IS NOT NULL THEN total_score := total_score + NEW.S_3; score_count := score_count + 1; END IF;
    IF NEW.S_4 IS NOT NULL THEN total_score := total_score + NEW.S_4; score_count := score_count + 1; END IF;
    IF NEW.S_5 IS NOT NULL THEN total_score := total_score + NEW.S_5; score_count := score_count + 1; END IF;
    IF NEW.S_6 IS NOT NULL THEN total_score := total_score + NEW.S_6; score_count := score_count + 1; END IF;
    IF NEW.S_7 IS NOT NULL THEN total_score := total_score + NEW.S_7; score_count := score_count + 1; END IF;
    IF NEW.S_8 IS NOT NULL THEN total_score := total_score + NEW.S_8; score_count := score_count + 1; END IF;
...
CREATE TRIGGER trigger_calculate_summative_average
