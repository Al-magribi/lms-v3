...
const classes = [
	{
		...
		major: "id jurusan",
		name: "nama kelas",
		grade_name: "nama tingkat",
		major_name: "Nama jurusan",
		students:
			"jumlah siswa pada table cl_students.classid yang sama dengean id kelas",
	},
]
	...
	try {
		...
		if (checkStudent.rowCount > 0) {
			const checkClass = await client.query(
				`SELECT * FROM a_class WHERE id = $1`,
				[checkStudent.rows[0].classid]
			)
			await client.query("ROLLBACK")
		...
		await client.query(
			`INSERT INTO cl_students(periode, classid, student, student_name)
			VALUES($1, $2, $3, $4)`,
			[periodeid, classid, studentid, checkData.rows[0].name]
		)
	...
	try {
		...
		const studentData = await Promise.all(
			students.map(async (student) => {
				const user = await client.query(
					`SELECT id, name FROM u_students WHERE nis = $1`,
					[student[0]]
				)
		...
		const alreadyRegistered = await Promise.all(
			validStudents.map(async (student) => {
				const checkStudent = await client.query(
					`SELECT classid FROM cl_students WHERE student = $1`,
					[student.id]
				)

				if (checkStudent.rowCount > 0) {
					const checkClass = await client.query(
						`SELECT name FROM a_class WHERE id = $1`,
						[checkStudent.rows[0].classid]
					)
					return {
		...
		await Promise.all(
			newStudents.map(async (student) => {
				await client.query(
					`INSERT INTO cl_students(periode, classid, student, student_name)
					VALUES($1, $2, $3, $4)`,
					[periodeid, classid, student.id, student.name]
				)
			})
		)
	...
	try {
		...
		const [count, data] = await Promise.all([
			...
			client.query(
				...
				LIMIT $4 OFFSET $5`,
				[classid, periodeid, `%${search}%`, limit, offset]
			),
		])
...
router.delete("/delete-student", authorize("admin"), async (req, res) => {
	...
	try {
		...
		await client.query("BEGIN")
		await client.query(`DELETE FROM cl_students WHERE id = $1`, [id])
		await client.query("COMMIT")
